        ...
        <div class="px-4 sm:px-6 lg:px-8 ">
            <div class="sm:flex sm:items-center">
                <div class="sm:flex-auto">
                    <h1 class="text-xl font-semibold text-gray-900">Titles</h1>
                </div>
            </div>
            <div class="mt-8 flex flex-col">
                <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                        <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                            ...
                            <div class="my-2">
                                ...
                                <form method="post" action="{{ route('guest.titlessearch') }}">

                                    @csrf

                                    <input type="text" name="search" id="search" value="" placeholder="Title"
                            ...
                            <table class="divide-y divide-gray-300" style="min-width: 32rem; width: 32rem;">
                                <caption>{{ $compositionstotalcount }} titles found.</caption>
                                <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold sm:pl-6" style="width: 70%;">
                                        <a href="{{ route('guest.titles', ['column' => 'titles', 'direction' => $titledirection]) }}"
                                           class="group inline-flex"
                                           style="@if($column === 'titles') color: blue @else color: black @endif"
                                        >
                                            Title
                    ...
                    @if(($column === 'titles') && ($direction === 'asc'))
                                    ...
                                    <tr style="{{ $loop->odd ? 'background-color: rgba(0,0,0,0.1)' : '' }}">
                                        ...
                                        <td class="whitespace-nowrap px-3 py-2 text-sm text-left">
                                            <div class="flex flex-col">
                                                @foreach($composition['artists'] AS $artist)
                                                    <div>{{ $artist->fullnameAlpha.' ('.$artist->artisttypeAbbr($artist->pivot->artisttype_id).')'}}</div>
                                                @endforeach
                                            </div>
                                        </td>
                                        <td class="whitespace-nowrap px-3 py-2 text-sm text-center">{{ $composition->performanceCount }}</td>
                                        <td class="whitespace-nowrap px-3 py-2 text-sm text-left">{!! $composition->performanceYearsCsv !!}</td>
                                    </tr>
                                @endforeach
