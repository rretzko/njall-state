<div class="flex flex-col text-center">
    ...
    <div id="search-box" class=" ">
        ...
        <input wire:model.debounce.500ms="search" class="text-indigo-600" type="text" placeholder="Enter school name" autofocus />
    </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        @foreach($items AS $key => $item)
                            <tr @if(($loop->iteration % 2)) style="background-color: rgba(0,0,0, .1);" @endif>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-black text-left">{{ $item->name }}</td>
                                <td class="px-2">
                                <select wire:model="eventschoolid" class="text-blueGray-800" key="{{ $item->id }}">
    ...
    <div id="conductors-program" class="mt-4">
        ...
        @if($event) <x-programs.template-primary :event="$event" :participants="$participants" :schoolid="$school->id"/> @endif
