    ...
    @foreach($event->ensembles AS $ensemble)
        @if($ensemble->participantsCount($event))
            ...
            <header class="bg-blueGray-300 font-bold mb-2" >
                {{ $event->year_of }} {{ $ensemble->name }} Participants ({{ $ensemble->participantsCount($event) }})
            </header>
            ...
            <div x-data="{ active: {{ $ensemble->instrumentations->first()->id }} }" class="space-y-4">
                ...
                >
                    <h2>
                        ...
                        >
                            <span style="@if($participant && ($participant->instrumentation_id == $instrumentation->id)) background-color: yellow; @endif " >
                                {{ $instrumentation->descr }} ({{ $ensemble->participantsByInstrumentationCount($event, $instrumentation) }})
                            </span>
                    ...
                    <div x-show="expanded" x-collapse.duration.1500ms>
                        <div class="pb-4 px-6 flex flex-wrap">
                            {!! $ensemble->participantsByInstrumentationBlocks($event, $instrumentation,10,$participant) !!}
                        </div>
                    </div>
                </div>
            </div>
